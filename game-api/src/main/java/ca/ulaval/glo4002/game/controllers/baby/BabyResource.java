...
import ca.ulaval.glo4002.game.controllers.baby.dtos.ExternalApiBabyListDto;
...
public class BabyResource {
    ...
    public Response createBebe(BabyRequest babyRequest) {
        ...
        externalApiCreationDto = bebeUseCase.getParentsSpecies(babyRequest.getFatherName(), babyRequest.getMotherName());
        ...
        if (response.getStatusInfo().getFamily().name().equals("SUCCESSFUL")) {
            ExternalApiBabyListDto externalApiBabyListDto = response.readEntity(ExternalApiBabyListDto.class);
            BabyCreationDto dto = babyDtoAssembler.fromRequest(babyRequest, externalApiBabyListDto);
